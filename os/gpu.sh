...
function os_create() {
  ...
  openstack server create --image $2 --flavor $FLAVOR_ID --network demo-net --key-name my-key -f shell $1 | tee .$1
  while true; do
    echo -n .
    . .$1
    if [ "$status" != "BUILD" ]; then break; fi
    sleep 1
    openstack server show -f shell $id > .$1
  done
...
# os_recreate win-gpu  win10-gpu  m1.medium.$GPU  10.44.96.61
os_recreate win  win10-gpu  m1.medium  10.44.96.54
